...
Motivation: 现有偏见评估数据集主要针对英语和北美文化，且无法从多角度评估LLMs的偏见，中文文化背景下的数据集稀缺。
...
TL;DR: 研究了Huginn-3.5B模型在算术任务中是否存在潜在推理结构，发现其内部推理步骤的可解释性有限且不一致。


<details>
  <summary>Details</summary>
Motivation: 探索深度循环Transformer是否能在潜在空间中形成推理结构，以提升效率。

Method: 使用Logit Lens和Coda Lens等技术分析模型在算术任务中的内部行为。

Result: 发现潜在推理步骤的可解释性有限，且不同循环块之间存在显著不一致。

Conclusion: 增加循环深度仅带来边际收益，显式推理步骤模型表现更优。
...
Method: 开发并评估了多种大型语言模型（LLMs），用于将用户输入的自然语言描述自动转换为GDC队列过滤器，并提供交互式界面供用户进一步优化。
...
TL;DR: MemAgent通过分段读取和覆盖策略优化长文本处理，在32K训练下实现8K到3.5M的扩展，性能损失<5%，512K测试达95%+。
...
Result: 在32K训练下，8K到3.5M的扩展性能损失<5%，512K测试达95%+。
...
Method: 利用LoRA模块实现高效、并行且对领域顺序鲁棒的DAP，同时为特定任务提供定制模型。

Result: DoMIX在DAP和标准LLM微调场景中均表现出色，代码已开源。

Conclusion: DoMIX通过LoRA模块解决了持续DAP的关键问题，并展示了其广泛适用性。
...
TL;DR: 本文介绍了用于SciVQA 2025共享任务的系统，结合多模态大语言模型和少样本检索策略，根据问题类型选择模型和答案，最终在测试中排名第三。
...
Method: 使用两个多模态大语言模型和少样本检索策略，根据问题类型和模型置信度选择答案。
...
TL;DR: QFFN-BERT是一种混合量子-经典Transformer模型，用参数化量子电路（PQC）替换BERT的FFN模块，减少参数量的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: FFN模块在Transformer中占主导参数量（约2/3），PQC可能成为更高效且表达能力强的替代方案。

Method: 设计包含残差连接、$R_Y$和$R_Z$旋转及交替纠缠策略的PQC架构，系统研究深度、表达能力和可训练性的权衡。

Result: 在SST-2和DBpedia基准测试中，QFFN-BERT达到基线102%准确率，FFN参数减少99%，并在少样本学习中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法依赖大量数据提升模型性能，但忽视数据质量，导致训练效率低下。

Method: 利用参数化模型选择代码数据，确保子集的分布一致性和多样性。
...
TL;DR: 研究评估了七种基于Transformer架构的Akan ASR模型在四种不同领域的语音数据集上的表现，发现模型性能高度依赖训练领域，并比较了Whisper和Wav2Vec2的错误行为差异。
...
Method: 使用四种Akan语音数据集（涵盖文化描述、非正式对话、圣经诵读和金融对话）对七种Transformer架构的ASR模型进行基准测试，分析词错误率和字符错误率。
...
TL;DR: 该研究提出了一种收集语音样本的方法，用于构建针对低资源语言和受损语音的自动语音识别（ASR）模型，并通过开发“最佳实践手册”和培训，推动社区驱动的数据收集和模型构建。


<details>
  <summary>Details</summary>
Motivation: 旨在民主化ASR技术和数据收集，特别是为受损语音和低资源语言（如加纳的阿坎语）提供支持。

Method: 开发了“最佳实践手册”和培训材料，并收集了阿坎语受损语音的首个开源数据集，参与者来自多样化背景。

Result: 公开了数据集、手册和开源工具，并展示了针对阿坎语受损语音的ASR模型微调初步结果。

Conclusion: 该研究为研究人员和从业者提供了工具和资源，以开发更包容的ASR技术，满足受损语音个体的独特需求。
...
<details>
  <summary>Details</summary>
Motivation: 超越人类认知限制是LLM训练的关键前沿，专有代理系统（如DeepResearch）在复杂信息搜索任务中展现了超人类能力，开源模型缺乏这种能力。
...
TL;DR: 论文探讨了高质量标注数据的限制问题，提出人类标签变异（HLV）作为信息信号的重要性，并提出了一个结合HLV的主动学习框架。


<details>
  <summary>Details</summary>
Motivation: 解决监督学习中高质量标注数据的稀缺问题，并利用人类标签变异（HLV）作为有用信号，而非噪声。

Method: 通过分解标签变异为信号（HLV）和噪声（如标注错误），提出一个结合HLV的主动学习框架，包括实例选择、标注者选择和标签表示。

Result: 提出了一个概念框架，支持在主动学习中整合HLV，并探讨了大型语言模型（LLM）作为标注者的可能性。
...
Conclusion: MPF是一种兼容已部署LLM的轻量级偏见缓解方法，具有可扩展性和可解释性。
...
TL;DR: 研究探讨了性别与语境偏见的关联，引入新数据集GenderLexicon和评估框架，证实了性别偏见的存在并提升了其可解释性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索性别与语境偏见的关联，特别是职业相关偏见。

Method: 方法包括引入GenderLexicon数据集和评估框架，通过评分解释偏见。

Result: 结果证实了性别偏见的存在，并在五个数据集中验证了方法的有效性。

Conclusion: 结论表明该方法能有效评估和解释性别偏见，扩展了偏见研究的范围。
...
<details>
  <summary>Details</summary>
Motivation: 同行评审是科研的核心，但日益增长的论文数量加剧了其挑战。LLMs在科学任务中表现潜力，但其在辅助同行评审（尤其是识别局限性）方面的潜力尚未充分研究。
...
Conclusion: LimitGen为LLMs在辅助同行评审中识别局限性提供了有效工具，增强了其反馈的具体性和建设性。
...
TL;DR: 论文研究了大型语言模型（LLMs）在自我纠正方面的局限性，提出了一个系统性框架（Self-Correction Bench）来测量这一问题，并发现简单的干预措施（如添加“Wait”）能显著改善模型表现。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs具有变革性，但其在自我纠正方面存在系统性盲点（Self-Correction Blind Spot），影响其可靠性和可信度。研究旨在揭示这一现象并提出改进方法。

Method: 通过Self-Correction Bench框架，在三个复杂度级别上注入控制错误，测试了14个模型，并分析了训练数据组成的影响。

Result: 平均盲点率为64.5%，研究发现训练数据中缺乏错误纠正序列是主要原因。简单干预（如添加“Wait”）可将盲点率降低89.3%。

Conclusion: 研究揭示了当前LLMs的局限性，并提出了通过调整训练数据或简单干预来提升其可靠性和可信度的潜在途径。
...
<details>
  <summary>Details</summary>
Motivation: 探讨推理能力对语言模型在偏见和安全性的影响，明确推理机制是否真正提升了模型的鲁棒性。

Method: 使用CLEAR-Bias基准评估RLMs，采用LLM-as-a-judge方法进行自动化安全评分，并结合越狱技术测试安全机制。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型应用中需平衡性能和成本，研究发现58%医疗问题无需高成本推理即可准确回答，表明动态路由可优化效率。
...
TL;DR: 论文提出IFBench，一个评估语言模型精确遵循指令泛化能力的新基准，并展示了通过强化学习与可验证奖励（RLVR）显著提升模型性能。
...
Method: 设计IFBench基准和约束验证模块，采用强化学习与可验证奖励（RLVR）训练模型。

Result: RLVR显著提升了模型在精确指令遵循任务中的泛化能力。

Conclusion: IFBench和RLVR为提升语言模型精确指令遵循能力提供了有效工具。
...
TL;DR: 论文揭示了一种语言模型（LM）的漏洞，即单个用户仅通过提供提示和点赞/点踩反馈，就能持续改变LM的知识和行为。攻击者通过操纵反馈信号，使模型在后续偏好调整中更倾向于生成恶意响应。
...
Method: 攻击方法包括让模型随机输出“中毒”或良性响应，并通过点赞中毒响应或点踩良性响应来操纵反馈信号。

Result: 结果显示，攻击可用于插入虚假事实、修改代码生成模式以引入安全漏洞，以及注入虚假金融新闻。

Conclusion: 结论指出，该研究不仅揭示了偏好调整的新特性，还扩展了关于预训练数据中毒和部署时提示注入的攻击机制。
...
TL;DR: MOTIF是一种通过强化学习微调的模块化思考方法，用于扩展大语言模型的上下文限制，提升推理能力。
...
Method: 提出MOTIF方法，通过强化学习微调生成多轮思考标记，扩展上下文。在GSM8K数据集上训练Qwen2.5-3B-Instruct模型。

Result: 在MATH500和AIME2024基准测试中，分别比传统GRPO方法提升了3.8%和3.3%，且仅需15%的样本。

Conclusion: MOTIF方法有效提升了模型的推理能力，并具有样本高效性。
...
TL;DR: 论文指出多项选择题评测存在缺陷，提出通过生成式答案匹配评测方法，并验证其与人工评分的一致性优于传统方法。
...
Method: 提出答案匹配方法：模型生成自由答案，再用语言模型判断是否与参考答案匹配。

Result: 答案匹配方法与人工评分一致性接近完美，优于多项选择题评测和LLM-as-a-judge方法。

Conclusion: 答案匹配方法更可靠，建议从多项选择题评测转向该方法。
...
Conclusion: 领域适应和尺度感知监督对水下环境中鲁棒的单目深度估计至关重要。
...
Method: 采用EVA-CLIP编码事件流，结合Llama分词器和Vicuna-7B大语言模型，通过Q-former对齐视觉和语言特征，输出答案和推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 图形摘要在科学传播中具有重要作用，但设计高质量GA需要专业技能，且其潜力尚未充分挖掘。
...
<details>
  <summary>Details</summary>
Motivation: 工业视觉系统中高质量训练数据收集耗时，合成数据成为关键解决方案，但现有方法生成速度慢且仿真与现实差距大。扩散模型能快速生成高质量图像，但精确控制仍困难。

Method: 研究80个视觉概念，比较基于提示和基于布局的两种条件策略。

Result: 当条件线索匹配完整训练分布时，合成数据比单独使用真实数据平均提高34%的平均精度，最高达177%。

Conclusion: 布局条件策略在多样性高时表现更优，为低数据场景下的物体检测提供了高效解决方案。
...
Result: 在Occ3D-nuScenes和OpenOcc数据集上表现优异，两帧输入的FMOcc分别达到43.1% RayIoU和39.8% mIoU，推理内存5.4G，时间330ms。
...
TL;DR: SurgVisAgent是一个基于多模态大语言模型（MLLMs）的智能手术视觉代理，能够动态识别内窥镜图像的失真类别和严重程度，执行多种增强任务，超越传统单任务模型。
...
TL;DR: 提出了一种名为MAC-Lookup的模型，通过改进颜色准确性、锐度和对比度来增强水下图像质量。
...
Result: LaCo在视觉编码器中间层压缩令牌时优于现有方法，训练效率提升20%以上，推理吞吐量提升15%以上。
...
Method: 引入视觉上下文提示编码器和动态目标对齐适配器，无需手动提示即可生成对象感知提示并注入类特定语义。
...
TL;DR: DreamComposer++ 是一个改进多视角条件控制的框架，通过提取和融合多视角3D表示，提升预训练扩散模型生成可控新视角的能力。
...
Method: 利用视角感知的3D提升模块提取多视角3D表示，通过多视角特征融合模块聚合并渲染到目标视角的潜在特征中，再集成到预训练扩散模型中进行新视角合成。

Result: 实验表明，DreamComposer++ 能无缝集成先进视角感知扩散模型，提升其生成可控新视角的能力。
...
Method: Flow-CDNet由光流分支和二进制变化检测分支组成，前者通过金字塔结构提取多尺度位移变化，后者结合ResNet和光流分支输出生成快速变化结果。
...
Method: 提出leaky max pooling (LMP)层和选择策略，结合注意力掩码和可学习聚类层，构建LMPNet模型。

Result: LMPNet能自动发现鲁棒的关键点，性能接近有监督的姿态估计模型。

Conclusion: LMPNet通过直接操作网络滤波器实现关键点检测，具有高可解释性和实用性。
...
TL;DR: 论文提出了一种利用fMRI信号增强视觉解码的方法，通过跨注意力机制将fMRI表征注入多尺度图像特征，提高了下游检测和分割任务的准确性。
...
Result: 实验表明，加入fMRI信号能提升下游检测和分割的准确性，证实fMRI包含丰富的多对象语义线索和粗粒度空间定位信息。

Conclusion: fMRI信号中蕴含的语义和空间信息尚未被现有模型充分利用，通过跨注意力机制可以更有效地整合这些信息。
...
TL;DR: MAGIC是一种基于扩散模型的少样本异常生成方法，通过多级扰动和上下文感知对齐，解决了背景保持、掩码对齐和语义合理性问题。
...
Conclusion: MAGIC通过多级扰动和上下文感知对齐，有效解决了少样本异常生成的核心挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前T2V合成的评估指标主要关注视觉质量和时间一致性，缺乏对下游任务（如TVR）性能的深入洞察。

Method: 基于800个多样化的用户查询生成合成视频，并标注视频-文本对的四个语义对齐维度。开发Auto-Evaluator估计对齐质量。

Result: SynTVA不仅作为基准，还能用于数据集增强，显著提升TVR性能。

Conclusion: SynTVA为合成视频的评估和下游任务应用提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法（如ControlNet和ControlNet++）在空间控制上仍有不足，尤其是忽略了中间生成阶段的影响。

Method: 通过训练轻量级卷积探针，从每个去噪步骤的UNet特征中重建输入控制信号，并利用对齐损失优化整个扩散过程的控制一致性。

Result: InnerControl在多种控制方法（如边缘、深度）上实现了最先进的性能。

Conclusion: InnerControl通过全过程的控制优化，显著提升了生成图像的控制保真度和质量。
...
TL;DR: 该研究提出了一种基于人工神经网络（ANN）的图像处理技术，用于水稻叶部病害的早期分类和识别，并通过比较特征分析检测模型（FADM）和直接图像中心检测模型（DICDM），验证了FADM的优越性。
...
Conclusion: FADM在水稻叶部病害检测中具有显著潜力，可改善作物健康、减少产量损失并提升水稻种植的可持续性。
...
TL;DR: Hita是一种新型图像分词器，通过整体到局部的分词方案和关键策略改进自回归图像生成，显著提升训练速度和生成质量。
...
Method: 提出Hita分词器，结合可学习的整体查询和局部补丁标记，采用因果注意力和轻量级融合模块优化信息流。

Result: 在ImageNet基准测试中达到2.59 FID和281.9 IS，训练速度更快，并在零样本风格迁移和图像修复中表现优异。

Conclusion: Hita通过全局信息捕捉和优化信息流，显著提升了自回归图像生成的性能和应用范围。
...
Method: 1) 将动态场景分解为基于种子的局部空间；2) 解耦静态与动态特征，生成时间高斯模型。
...
TL;DR: 论文提出了一个水下观测基准UVLM，通过结合人类专业知识和AI模型构建数据集，解决了现有视频语言模型（VidLMs）在陆地场景中的局限性，并设计了多样化的任务和评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型主要关注陆地场景，忽视了水下观测的高需求应用。为填补这一空白，作者构建了UVLM基准，以提升水下世界的理解能力。

Method: 通过选择典型水下挑战的视频（如光线变化、水质浑浊和多视角）构建数据集，涵盖多样化的帧率、分辨率、海洋生物类别和静态植物地形。任务设计分为生物和环境两大类，共20种任务类型。

Result: 实验表明，在UVLM上微调的VidLMs显著提升了对水下世界的理解，同时对现有空中VidLM基准（如VideoMME和Perception text）也有轻微改进。

Conclusion: UVLM为水下观测提供了全面的基准，展示了其在提升VidLMs水下理解能力方面的潜力，数据集和提示工程将公开。
...
TL;DR: CoMEL是一个持续多实例学习框架，通过高效实例编码、可靠伪标记和遗忘缓解技术，显著提升WSI数据集的分类和定位性能。
...
Result: 在三个WSI数据集上，CoMEL在分类和定位任务中分别比现有方法提升11.00%和23.4%。

Conclusion: CoMEL为持续多实例学习提供了高效解决方案，显著提升了定位和分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于空间频率的检测方法忽略了像素级的时间不一致性，导致无法有效检测时间伪影。

Method: 对每个像素在时间轴上进行1D傅里叶变换提取特征，引入注意力提议模块和联合变换模块整合时空上下文特征。

Result: 该方法在多样化且具有挑战性的检测场景中表现出鲁棒性能。
...
Result: 在ACDC和MSCMR seg数据集上的实验表明，TAB Net优于现有弱监督方法，性能接近全监督方法。
...
TL;DR: 论文研究了自监督学习在野生动物重识别中的应用，通过无监督方式从相机陷阱数据中提取图像对训练模型，实验表明自监督模型在数据有限时更鲁棒，且在下游任务中表现优于监督学习。


<details>
  <summary>Details</summary>
Motivation: 当前野生动物重识别依赖标注数据，而自监督学习能利用无标注数据，减少对标注的依赖，提升模型性能。

Method: 从相机陷阱数据中自动提取时间图像对，训练自监督模型，并与监督学习特征在开放世界场景和下游任务中进行比较。
...
TL;DR: 该论文提出了一种结合扩散模型和自动编码器的方法，通过学习图像特定表示和组织潜在空间，用于脑图像分析、撕裂伪影修复和JPEG伪影修复。
...
Method: 1. 结合编码器与扩散模型，建立自动编码框架；2. 设计机制约束潜在空间以识别脑图像中的区域特定模式；3. 提出基于邻域感知的无监督撕裂伪影修复技术；4. 利用扩散模型的可控生成能力实现无监督JPEG伪影修复。

Result: 1. 潜在空间能区分脑组织类型；2. 撕裂伪影和JPEG伪影修复效果良好。

Conclusion: 通过结合扩散模型和自动编码器，论文实现了图像特定表示学习和潜在空间组织，为脑图像分析和伪影修复提供了有效方法。
...
TL;DR: 论文提出了一种针对热图像中行人跟踪的新型调优方法，通过优化两阶段框架和超参数，提高了跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 热图像在低能见度或弱光条件下优于RGB相机，但其低层次特征表示导致行人检测和跟踪困难。

Method: 采用两阶段调优框架，优化超参数以适应热图像中的复杂运动模式。

Result: 在PBVS Thermal MOT数据集上验证，方法在各种热相机条件下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 随着密文模板库规模增大，人脸检索过程耗时增加，隐私保护需求推动高效解决方案的开发。
...
Method: 采用残差U-Net模型的不同配置，并将其集成到一个包含卷积块的元模型中，形成集成模型。
...
TL;DR: AvatarMakeup是一种基于预训练扩散模型的3D化妆方法，解决了现有方法在动态表情和多视角下化妆效果不一致的问题，通过粗到细的策略实现高质量化妆效果。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯编辑方法在实现真实化妆效果时存在不足，无法保证动态表情和多视角下的一致性、身份保持和细节控制。

Method: 采用粗到细策略，利用扩散模型生成化妆图像作为监督，提出Coherent Duplication方法确保一致性，并通过Refinement Module提升细节质量。
...
TL;DR: 论文提出了一种自由形式测试时间适应（F²TTA）任务，并设计了图像级解耦提示调优（I-DiPT）框架，以应对临床实践中数据以任意长度和随机顺序到达的挑战。


<details>
  <summary>Details</summary>
Motivation: 由于数据标注成本高，测试时间适应（TTA）成为适应未见医疗站点的解决方案，但现有方法假设数据以完整域单位到达，而临床实践中数据通常以片段形式随机到达。

Method: 提出I-DiPT框架，使用图像不变提示和图像特定提示，并结合不确定性导向掩码（UoM）和平行图蒸馏（PGD）方法。

Result: 在乳腺癌和青光眼分类任务中，该方法优于现有TTA方法。

Conclusion: I-DiPT框架有效解决了自由形式数据片段中的适应问题，提升了模型性能。
...
Method: 结合多模态大语言模型进行角色提取、对话分析和叙事摘要，并通过场景级分割将编辑过程分解为三个子任务：亮点检测、开头/结尾选择和无关内容修剪。

Result: 实验结果表明，HIVE在通用和广告导向的编辑任务中均优于现有基线，显著缩小了自动与人工编辑视频的质量差距。

Conclusion: HIVE框架通过多模态叙事理解提升了视频编辑的连贯性和质量，为相关研究提供了新的数据集和基准。
...
TL;DR: Mesh Silksong是一种高效的自回归多边形网格表示方法，通过减少顶点重复和优化几何属性，实现了显著的压缩率和几何完整性提升。
...
Conclusion: Mesh Silksong是一种高效且几何优化的网格表示方法，适用于实际应用。
...
Method: 提出VisCo Attack，采用四种视觉策略构建上下文对话，动态生成辅助图像，并结合毒性模糊和语义优化生成攻击提示。

Result: VisCo在MM-SafetyBench上对GPT-4o的毒性得分为4.78，攻击成功率为85%，显著优于基线方法。

Conclusion: VisCo Attack为视觉模态的安全漏洞提供了新的研究方向，并展示了其在实际攻击中的高效性。
...
Result: 数据集提供了复杂场景下的完整标注，测试了多种SOTA模型及基础模型的表现。

Conclusion: CrowdTrack为复杂场景下的多行人跟踪算法研究提供了有力支持，数据集和代码已开源。
...
Result: 碎片化率比现有方法降低了2.2倍，性能达到新最优。

Conclusion: 时间感知对息肉计数至关重要，新方法显著提升了聚类鲁棒性。
...
Conclusion: MC-INR通过灵活的结构设计和动态优化，显著提升了复杂数据处理的性能。
...
Conclusion: 自动化红外-RGB标注流程能有效提升低光条件下的行人检测性能。
...
TL;DR: 本文提出了一种基于深度学习的解决方案，用于检测多种作物中的多种疾病，旨在覆盖印度多样化的农业景观。通过构建包含17种作物和34种疾病的统一数据集，提出的深度学习模型在准确性和覆盖范围上优于现有技术。
...
Result: 显著降低人工误差，姿态估计mAP达97.94%，像素到厘米转换误差为0.07 cm。
...
TL;DR: MoGe-2是一种先进的开放域几何估计模型，从单张图像恢复场景的度量尺度3D点图。它在MoGe基础上扩展，实现了度量几何预测，并通过数据细化方法提升细节恢复能力。
...
Conclusion: MoGe-2首次同时实现了高精度相对几何、精确度量尺度和细粒度细节恢复，超越了以往方法。
...
Method: 采用扩散模型学习人类社交距离和姿态先验，结合双分支优化框架和多种约束（如3D高斯、2D关键点等）重建动作和外观。
...
TL;DR: 提出了一种将体素、网格和参数化模型通过可微分转换结合的统一框架，用于血管建模。
...
Method: 通过可微分体素化和形状到分割的拟合，自动提取血管参数化模型（中心线和半径），并使用B样条保证平滑性。网格可从参数化模型可微分提取。

Result: 实验证明，该方法能准确捕捉复杂血管（如主动脉、动脉瘤和脑血管）的几何形状。

Conclusion: 该框架成功统一了三种表示方法，并实现了高保真度的血管建模。
...
TL;DR: 本文提出了一种名为$S^2DC$的自监督学习框架，通过考虑解剖结构的变异性，实现了结构感知的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有的3D医学图像自监督学习方法通常忽略解剖结构在位置、尺度和形态上的变异性，导致无法有效捕捉有意义的区分特征。

Method: $S^2DC$框架通过两个步骤实现结构感知表示：1) 利用最优传输策略增强不同结构间的语义差异性；2) 基于邻域相似性分布提升结构内的语义一致性。

Result: 在10个数据集、4个任务和3种模态上的实验表明，$S^2DC$显著优于现有方法。

Conclusion: $S^2DC$通过结合结构变异性，为3D医学图像自监督学习提供了更有效的表示方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决长视频理解中Transformer模型的高计算复杂性和内存成本问题。

Method: 用线性RNN语言模型替代LLM，结合视觉令牌合并和排序，实现高效处理。
...
TL;DR: 论文探讨了视觉导航算法在太空任务中的可靠性问题，提出通过AI检测传感器故障，并开发了一个模拟框架生成故障图像数据集。


<details>
  <summary>Details</summary>
Motivation: 视觉导航算法在太空任务中的重要性日益增加，但传感器故障可能导致导航失效，传统故障检测方法存在局限性，AI提供了一种潜在解决方案。

Method: 研究分析了相机传感器在视觉导航中的潜在故障案例，开发了一个模拟框架生成合成故障图像数据集。

Result: 提出了一个包含故障注入图像的模拟数据集，用于训练和测试AI故障检测算法。
...
TL;DR: 论文提出Holmes-Set数据集和Holmes Pipeline框架，用于检测AI生成图像（AIGI）并生成可验证的解释，解决了现有技术缺乏解释性和泛化性的问题。


<details>
  <summary>Details</summary>
Motivation: AI生成图像（AIGI）的滥用威胁公共信息安全，现有检测技术缺乏可验证解释和泛化能力。
...
Conclusion: Holmes-Set和Holmes Pipeline显著提升了AIGI检测的解释性和泛化能力。
...
TL;DR: 提出了一种结合深度展开和模型蒸馏的新框架，将扩散模型转化为条件模型，用于后验采样，提高了计算效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在计算成像中作为图像先验表现出色，但现有方法存在灵活性或精度不足的问题。
...
TL;DR: APT框架通过自适应训练策略和表示正则化，解决了扩散模型在有限数据下个性化训练中的过拟合问题，同时保持了先验知识和文本对齐。
...
Method: APT框架包含三个组件：自适应训练调整、表示稳定化和注意力对齐，分别通过动态调整训练策略、正则化特征图和对齐注意力图来优化模型。

Result: 实验表明，APT有效减少了过拟合，保留了先验知识，并在有限参考数据下生成高质量、多样化的图像。

Conclusion: APT为有限数据下的扩散模型个性化训练提供了一种高效且鲁棒的解决方案。
...
TL;DR: CanonSwap是一个新颖的视频换脸框架，通过解耦面部外观和运动信息，解决了现有方法在保持目标面部动态属性上的不足。
...
Method: CanonSwap通过将运动信息与外观信息解耦，先在统一规范空间内修改身份，再将特征重新整合回原始视频空间。此外，设计了部分身份调制模块和细粒度同步指标。

Result: 实验表明，CanonSwap在视觉质量、时间一致性和身份保持方面显著优于现有方法。

Conclusion: CanonSwap通过解耦和精细调制，实现了高质量的视频换脸，同时保持了目标面部的动态属性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D-3D特征对齐，导致3D理解能力受限和语义信息丢失，因此提出无需对齐的框架。

Method: SIU3R通过像素对齐的3D表示和统一的可学习查询，实现重建与理解的协同，并设计了轻量模块促进任务交互。

Result: 实验表明SIU3R在3D重建、理解及两者联合任务上均达到最先进性能。

Conclusion: SIU3R的无对齐框架和任务协同设计有效提升了性能。
...
Method: 设计UniMC框架，将实例和关键点条件整合为紧凑令牌；发布HAIG-2.9M数据集，包含丰富标注和严格人工检查。

Result: 实验证明HAIG-2.9M的高质量和UniMC的有效性，尤其在遮挡和多类场景中表现优异。

Conclusion: UniMC和HAIG-2.9M显著提升了关键点引导图像生成的灵活性和效果。
...
TL;DR: FairHuman通过多目标微调方法，提升文本到图像生成模型在全局和局部（如人脸和手）的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成人体图像时，局部细节（如人脸和手）的生成质量不足，主要因训练时缺乏对这些区域的充分监督。
...
TL;DR: 提出了一种基于边界框注释的弱监督方法，利用基础模型自动生成提示，减少医学图像分割中对像素级标注的依赖。


<details>
  <summary>Details</summary>
Motivation: 医学图像中像素级标注成本高昂且耗时，弱监督方法（如边界框标注）更实用。基础模型在分割任务中表现优异，但现有提示学习方法依赖全标注掩码。

Method: 结合基础模型的表示能力和弱监督分割的标注效率，通过边界框注释自动生成提示，并整合伪标签与多约束优化。

Result: 在多模态数据集上，该方法在有限数据设置下平均Dice得分为84.90%，优于现有全监督和弱监督方法。

Conclusion: 该方法显著减少标注需求，同时保持高性能，为医学图像分割提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前研究主要集中在简单夹具控制，缺乏针对灵巧手的功能性抓取研究，数据收集难度限制了进展。

Method: 生成包含1.7亿灵巧抓取姿态的数据集DexGraspNet 3.0，训练视觉语言模型和基于流匹配的姿态预测头。

Result: 在仿真和真实实验中，DexVLG表现出强大的零样本泛化能力，抓取成功率达76%以上，并实现部分对齐抓取。

Conclusion: DexVLG为灵巧手抓取任务提供了高效解决方案，展示了大规模数据训练模型的潜力。
...
TL;DR: 提出了一种基于多极注意力神经算子（MANO）的线性复杂度Transformer变体，用于高效处理高分辨率输入。
...
Method: 将注意力建模为网格点间的交互问题，引入多尺度距离计算的多极注意力（MANO），实现线性复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 解决定向目标检测中高标注成本问题，探索部分弱标注的有效利用。
...
<details>
  <summary>Details</summary>
Motivation: 传统损害评估方法主观性强且无法处理图像内不同程度的损害，因此需要一种更客观的方法来提升灾害侦察的精确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有特征注入方法在结构对齐和条件泄漏方面存在问题，尤其是当条件图像与自然RGB分布差异较大时。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统分割模型依赖大规模标注数据和手动提示的问题，探索仅需少量参考图像的分割任务。
...
Conclusion: 该方法通过利用语义先验和自动生成分割掩码，显著减少了人工干预，实现了高性能的分割效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在静态面部表现良好，但在可动画化头像中仍存在细节不足的问题，尤其是在非线性变形和复杂光照下。

Method: 通过扩展3D高斯至高维多变量高斯（HyperGaussians），并引入局部嵌入学习和逆协方差技巧，提升表达能力和计算效率。

Result: 在4个面部数据集上的19名受试者中，HyperGaussians在数值和视觉上均优于3DGS，尤其在高频细节（如眼镜框、牙齿）和复杂动作上表现突出。

Conclusion: HyperGaussians通过高维表示和高效计算，显著提升了可动画化面部头像的质量，适用于增强和虚拟现实应用。
...
Conclusion: LangScene-X通过生成一致的新观测，实现了从稀疏视图重建和理解的通用性。
...
Result: 在四个公共基准数据集上进行了实验，验证了各组件有效性。
...
TL;DR: 论文提出了一种基于多注意力交互机制的综合优化方法，用于解决人类活动识别（HAR）中的标签数据稀缺、特征提取不足和设备性能问题。
...
Conclusion: 该方法显著提升了HAR性能，并在嵌入式设备上验证了其高效性和可行性。
...
TL;DR: 通过引入Grounded Chain-of-Thought (GCoT)方法，改进多模态大语言模型在数据有限情况下对专业视觉任务的适应能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在专业视觉任务（如图表理解）中表现不佳，原因是预训练数据与下游任务数据不匹配。

Method: 提出GCoT方法，通过在CoT数据中注入边界框信息，提高推理步骤的准确性。

Result: 在五种专业视觉任务上的实验表明，GCoT在数据有限情况下显著优于微调和蒸馏方法。

Conclusion: GCoT能有效提升模型在专业视觉任务中的表现，尤其在数据有限的情况下。
...
<details>
  <summary>Details</summary>
Motivation: 视频生成模型的广泛采用受到推理速度慢和计算成本高的限制，主要由于去噪过程的迭代性质。
...
Conclusion: LiteReality在真实扫描和公共数据集上表现优异，具有广泛的应用潜力。
...
Conclusion: RefTok能有效捕捉视频中的时间动态和上下文信息，显著提升性能。
...
### [110] [Analyzing and Improving Speaker Similarity Assessment for Speech Synthesis](https://arxiv.org/abs/2507.02176)
*Marc-André Carbonneau,Benjamin van Niekerk,Hugo Seuté,Jean-Philippe Letendre,Herman Kamper,Julian Zaïdi*

Main category: cs.SD

TL;DR: 论文研究了自动说话人验证（ASV）嵌入在捕捉语音身份时的局限性，发现其主要关注静态特征（如音色和音高范围），而忽略动态特征（如节奏）。作者提出U3D指标以评估动态节奏模式，并公开代码。


<details>
  <summary>Details</summary>
Motivation: 语音身份建模具有多面性，而当前ASV嵌入主要用于区分而非全面表征身份，因此需要研究其局限性。

Method: 分析ASV嵌入的捕捉能力，识别其忽略的动态特征，并提出U3D指标以补充评估。

Result: ASV嵌入主要关注静态特征，动态节奏被忽视；U3D能有效评估动态节奏模式。

Conclusion: 论文揭示了ASV嵌入的局限性，并提出U3D作为补充指标，为语音克隆系统中的身份一致性评估提供新思路。
...
### [111] [JoyTTS: LLM-based Spoken Chatbot With Voice Cloning](https://arxiv.org/abs/2507.02380)
*Fangru Zhou,Jun Zhao,Guoxin Wang*

Main category: cs.SD

TL;DR: JoyTTS是一个结合大型语言模型（LLM）和文本转语音（TTS）技术的端到端语音聊天机器人，具备语音克隆功能。基于开源模型MiniCPM-o和CosyVoice2，并在2000小时对话数据上训练。测试结果显示SS得分为0.73，WER为5.09。


<details>
  <summary>Details</summary>
Motivation: 结合LLM和TTS技术，开发具备语音克隆功能的端到端语音聊天机器人，以提升交互体验。
...
### [112] [ASDA: Audio Spectrogram Differential Attention Mechanism for Self-Supervised Representation Learning](https://arxiv.org/abs/2507.02666)
*Junyu Wang,Tianrui Wang,Meng Ge,Longbiao Wang,Jianwu Dang*

Main category: cs.SD

TL;DR: 论文提出了一种差分注意力机制（ASDA），通过双软最大操作和调优的差分系数，解决了标准Transformer注意力机制中无效分配的问题，并在多个音频任务中实现了SOTA性能。
...
### [113] [DeSTA2.5-Audio: Toward General-Purpose Large Audio Language Model with Self-Generated Cross-Modal Alignment](https://arxiv.org/abs/2507.02768)
*Ke-Han Lu,Zhehuai Chen,Szu-Wei Fu,Chao-Han Huck Yang,Sung-Feng Huang,Chih-Kai Yang,Chee-En Yu,Chun-Wei Chen,Wei-Chih Chen,Chien-yu Huang,Yi-Cheng Lin,Yu-Xiang Lin,Chi-An Fu,Chun-Yi Kuan,Wenze Ren,Xuanjun Chen,Wei-Ping Huang,En-Pei Hu,Tzu-Quan Lin,Yuan-Kuei Wu,Kuan-Po Huang,Hsiao-Ying Huang,Huang-Cheng Chou,Kai-Wei Chang,Cheng-Han Chiang,Boris Ginsburg,Yu-Chiang Frank Wang,Hung-yi Lee*

Main category: eess.AS

TL;DR: DeSTA2.5-Audio是一种通用的大型音频语言模型（LALM），通过自生成跨模态对齐策略解决传统方法中LLM语言能力遗忘问题，并在多个音频语言基准测试中表现优异。
...
Result: tubuleTracker分析时间最短（6秒），且其指标与血管生成成熟度显著相关。

Conclusion: tubuleTracker是高效且一致的工具，尤其适用于评估血管生成成熟度。
...
### [115] [STELLA: Self-Evolving LLM Agent for Biomedical Research](https://arxiv.org/abs/2507.02004)
*Ruofan Jin,Zaixi Zhang,Mengdi Wang,Le Cong*

Main category: cs.AI

TL;DR: STELLA是一种自进化AI代理，通过动态工具库和模板库提升能力，在生物医学任务中表现优异且随经验增长。
...
Method: 采用多代理架构，包括动态工具库和模板库，通过自主发现和集成新工具实现能力进化。

Result: 在多个生物医学基准测试中表现优异，分数显著高于现有模型，且性能随经验提升。
...
### [116] [Strategic Intelligence in Large Language Models: Evidence from evolutionary Game Theory](https://arxiv.org/abs/2507.02618)
*Kenneth Payne,Baptiste Alloui-Cros*

Main category: cs.AI

TL;DR: LLMs在迭代囚徒困境中表现出战略智能，不同模型展现独特行为模式。
...
Result: LLMs表现优异，展现独特战略行为：Gemini冷酷，OpenAI合作，Claude宽容。
...
### [117] [Decoupled Planning and Execution: A Hierarchical Reasoning Framework for Deep Search](https://arxiv.org/abs/2507.02652)
*Jiajie Jin,Xiaoxi Li,Guanting Dong,Yuyao Zhang,Yutao Zhu,Yang Zhao,Hongjin Qian,Zhicheng Dou*

Main category: cs.AI

TL;DR: HiRA提出了一种分层框架，将战略规划与专门执行分离，显著提升了复杂搜索任务的性能。
...
Conclusion: 分层规划与执行分离在多步信息搜索任务中表现出高效性和优越性。
...
### [118] [StepHint: Multi-level Stepwise Hints Enhance Reinforcement Learning to Reason](https://arxiv.org/abs/2507.02841)
*Kaiyi Zhang,Ang Lv,Jinpeng Li,Yongbo Wang,Feng Wang,Haoyuan Hu,Rui Yan*

Main category: cs.AI

TL;DR: StepHint是一种新的RLVR算法，通过多级逐步提示解决近失奖励问题和探索停滞，提升模型训练效率和推理能力。
...
Result: StepHint在六个数学基准测试中表现优异，泛化能力强，且在域外基准测试中优于基线方法。
...
### [119] [Multimodal Misinformation Detection Using Early Fusion of Linguistic, Visual, and Social Features](https://arxiv.org/abs/2507.01984)
*Gautam Kishore Shahi*

Main category: cs.LG

TL;DR: 研究探讨了多模态特征组合（文本、图像和社交特征）在社交媒体虚假信息检测中的有效性，通过早期融合方法提升分类模型性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体在选举和危机期间充斥大量虚假信息，现有研究多关注单一模态（文本或图像），多模态特征组合的研究较少。
...
### [120] [Evaluating the Promise and Pitfalls of LLMs in Hiring Decisions](https://arxiv.org/abs/2507.02087)
*Eitan Anzenberg,Arunava Samajpati,Sivasankaran Chandrasekar,Varun Kacholia*

Main category: cs.LG

TL;DR: 大语言模型（LLM）在招聘中的应用虽能简化筛选流程，但存在准确性和算法偏见问题。研究发现，专用招聘模型（Match Score）在准确性和公平性上优于通用LLM。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在招聘中的潜在偏见问题，并验证专用模型是否能同时实现高准确性和公平性。

Method: 对比多种LLM与专用招聘模型（Match Score）在10,000个真实招聘案例中的表现，评估准确性和公平性指标。

Result: Match Score在ROC AUC（0.85 vs 0.77）和公平性（如种族影响比0.957 vs 0.809）上显著优于通用LLM。

Conclusion: 专用模型能有效减少偏见，强调在高风险领域需结合领域特定建模和偏见审核，而非依赖未经优化的通用LLM。
...
### [121] [Energy-Based Transformers are Scalable Learners and Thinkers](https://arxiv.org/abs/2507.02092)
...
### [122] [OmniDraft: A Cross-vocabulary, Online Adaptive Drafter for On-device Speculative Decoding](https://arxiv.org/abs/2507.02659)
*Ramchalam Kinattinkara Ramakrishnan,Zhaocong Yuan,Shaojie Zhuo,Chen Feng,Yicheng Lin,Chenzheng Su,Xiaopeng Zhang*

Main category: cs.LG

TL;DR: OmniDraft提出了一种统一的框架，使单个草稿模型能与任何目标模型配合使用，并通过动态适应用户数据解决跨词汇不匹配问题，提升解码速度。
...
### [123] [ExPO: Unlocking Hard Reasoning with Self-Explanation-Guided Reinforcement Learning](https://arxiv.org/abs/2507.02834)
*Ruiyang Zhou,Shuozhe Li,Amy Zhang,Liu Leqi*

Main category: cs.LG

TL;DR: 论文提出了一种名为ExPO的新方法，通过结合当前策略和正确答案生成高质量样本，提升模型在推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习后训练方法依赖模型初始能力生成正样本，限制了其在早期训练和复杂推理任务中的应用。

Method: 提出Self-Explanation Policy Optimization (ExPO)，通过条件化正确答案生成高质量样本，促进模型探索新推理路径。

Result: ExPO在推理任务中表现优于基于专家示范的方法，尤其在模型初始表现较差的MATH level-5任务中。

Conclusion: ExPO通过高效探索和样本生成，显著提升了模型在复杂推理任务中的学习效率和最终性能。
...
### [124] [Generative Latent Diffusion for Efficient Spatiotemporal Data Reduction](https://arxiv.org/abs/2507.02129)
*Xiao Li,Liangji Zhu,Anand Rangarajan,Sanjay Ranka*

Main category: cs.LG

TL;DR: 提出了一种结合变分自编码器和条件扩散模型的高效潜在扩散框架，显著提高了数据压缩的性能。
...
Result: 实验结果显示，该方法在多个数据集上实现了比SZ3高10倍的压缩比，比领先的学习方法性能提升63%。
...
### [125] [Holistic Continual Learning under Concept Drift with Adaptive Memory Realignment](https://arxiv.org/abs/2507.02310)
*Alif Ashrafee,Jedrzej Kozal,Michal Wozniak,Bartosz Krawczyk*

Main category: cs.LG

TL;DR: 论文提出了一种名为AMR的轻量级方法，用于动态数据流中的持续学习，通过选择性更新记忆缓冲区来应对概念漂移，性能接近完全重新训练但计算成本更低。
...
Result: AMR在多个数据集上表现优异，接近完全重新训练的性能，但显著减少了计算和标注需求。
...
### [126] [L-VAE: Variational Auto-Encoder with Learnable Beta for Disentangled Representation](https://arxiv.org/abs/2507.02619)
*Hazal Mogultay Ozcan,Sinan Kalkan,Fatos T. Yarman-Vural*

Main category: cs.LG

TL;DR: L-VAE是一种新型模型，通过学习损失函数的超参数和模型参数，动态平衡解缠和重构损失，优于现有方法。
...
Method: 提出L-VAE模型，同时学习损失函数权重和模型参数，并添加正则化项防止偏置。

Result: L-VAE在多个数据集上表现优异，解缠和重构效果均优于或接近现有方法。

Conclusion: L-VAE有效平衡解缠和重构，适用于多种任务，尤其在面部属性解缠上表现突出。
...
### [127] [Fair Deepfake Detectors Can Generalize](https://arxiv.org/abs/2507.02645)
...
<details>
  <summary>Details</summary>
Motivation: 解决深度伪造检测中公平性与泛化性之间的冲突，揭示其因果关系。
...
<details>
  <summary>Details</summary>
Motivation: 小型组织和初创公司缺乏法律专业知识，手动提取法律文本的元数据以生成合规需求资源密集且复杂。现有方法未能充分考虑元数据属性间的交互关系，且依赖手动标注或启发式机器学习，泛化能力不足。

Method: 采用文本蕴含和上下文学习技术，设计了一个领域特定的元模型（Python类结构），自动生成法律文本的规范表示，减少对大规模标注数据的依赖。

Result: 在13个美国州的数据泄露通知法律上测试，生成的表示通过89.4%的测试用例，精确率和召回率分别为82.2和88.7。

Conclusion: 该方法通过自动化生成法律文本的规范表示，显著提高了合规性分析的效率和泛化能力，适用于未见过的法律文本。
...
TL;DR: 论文研究了如何利用GPT-4o生成需求访谈中的后续问题，以支持访谈者。实验表明，LLM生成的问题在清晰度、相关性和信息量上不亚于人类编写的问题，且在指导生成时表现更优。


<details>
  <summary>Details</summary>
Motivation: 需求访谈中，访谈者面临领域不熟悉、认知负荷和信息过载等挑战，需要实时生成合适的问题。LLMs在自然语言处理任务中的优异表现激发了其在此领域的应用潜力。

Method: 基于常见访谈错误类型的框架，利用GPT-4o生成后续问题，并通过两个对照实验评估LLM生成与人类编写的问题。

Result: LLM生成的问题在清晰度、相关性和信息量上与人类编写的问题相当，且在指导生成时表现更优。

Conclusion: LLMs有潜力实时提升需求访谈的质量和效率，尤其是在指导生成问题时。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是改进绿色专利的分类方法，以更准确地支持政策制定和企业决策。

Method: 使用神经网络扩展基础词典，通过向量表示与环境技术相关的表达，并结合企业财务数据验证专利的经济影响。

Result: ‘真正’的绿色专利占20%，持有此类专利的企业在销售、市场份额和生产率方面表现更好，高创新性专利还能提高利润。

Conclusion: 文本分析对精细专利分类至关重要，有助于多领域的政策制定。
...
### [131] [Why Multi-Interest Fairness Matters: Hypergraph Contrastive Multi-Interest Learning for Fair Conversational Recommender System](https://arxiv.org/abs/2507.02000)
...
### [132] [Early Signs of Steganographic Capabilities in Frontier LLMs](https://arxiv.org/abs/2507.02737)
*Artur Zolkowski,Kei Nishimura-Gasparian,Robert McCarthy,Roland S. Zimmermann,David Lindner*

Main category: cs.CR

TL;DR: 当前大型语言模型（LLM）具备初步的隐写能力，但尚不足以绕过设计良好的监控。


<details>
  <summary>Details</summary>
Motivation: 评估前沿LLM的隐写能力，以理解其潜在风险。

Method: 研究两种隐写方式：传递编码信息和执行编码推理。

Result: 当前模型在标准条件下无法隐藏短信息，但通过额外条件（如使用未监控的草稿）可能成功。模型在简单状态跟踪问题中表现出初步的编码推理能力。
...
TL;DR: FinAI-BERT是一种基于Transformer的语言模型，用于在金融文本中句子级别分类AI相关内容，性能优越且可解释性强。


<details>
  <summary>Details</summary>
Motivation: 现有方法在粒度、可解释性和鲁棒性上不足，需要更精细的工具检测AI披露。

Method: 使用FinAI-BERT模型，基于1,586个手动标注的句子进行微调。

Result: 模型准确率达99.37%，F1分数0.993，优于传统基线方法。

Conclusion: 研究为金融NLP提供了细粒度分类方法，并为监管和分析提供了实用工具。
...
### [134] [CineMyoPS: Segmenting Myocardial Pathologies from Cine Cardiac MR](https://arxiv.org/abs/2507.02289)
...
<details>
  <summary>Details</summary>
Motivation: 心肌梗死（MI）是全球主要死因之一，现有LGE和T2加权CMR成像技术耗时且有侵入性，而cine CMR快速且无对比剂，但需要新方法从中提取病理信息。

Method: CineMyoPS结合运动和结构特征，设计一致性损失函数促进联合学习，并提出时间序列聚合策略整合心脏周期中的MI相关特征。

Result: 在多中心数据集上，CineMyoPS在心肌病理分割、运动估计和结构分割中表现优异。

Conclusion: CineMyoPS为无对比剂、快速的心肌病理评估提供了有效解决方案。
...
### [135] [A robust and versatile deep learning model for prediction of the arterial input function in dynamic small animal $\left[^{18}\text{F}\right]$FDG PET imaging](https://arxiv.org/abs/2507.02367)
...
<details>
  <summary>Details</summary>
Motivation: 传统的小动物动脉血液采样方法复杂、耗时且具有终止性，限制了纵向研究。

Method: FC-DLIF通过空间和时间特征提取器直接从PET序列中预测动脉输入函数。

Result: 模型在[$^{18}$F]FDG数据上表现可靠，但对其他示踪剂（如[$^{18}$F]FDOPA和[$^{68}$Ga]PSMA）无效。

Conclusion: FC-DLIF为非侵入性动脉输入函数预测提供了可靠且灵活的替代方案。
...
### [136] [3D Heart Reconstruction from Sparse Pose-agnostic 2D Echocardiographic Slices](https://arxiv.org/abs/2507.02411)
...
<details>
  <summary>Details</summary>
Motivation: 2D超声图像难以准确估计心脏参数，3D超声又受限于分辨率和手动标注的高要求。

Method: 设计了一种交替优化2D切片3D姿态估计和3D集成的重建流程，利用隐式神经网络逐步优化3D心脏模型。
...
### [137] [MEGANet-W: A Wavelet-Driven Edge-Guided Attention Framework for Weak Boundary Polyp Detection](https://arxiv.org/abs/2507.02668)
*Zhe Yee Tan*
...
Conclusion: MEGANet-W通过无参数小波边缘引导，显著提升了分割精度。
...
TL;DR: 论文提出FUSE，一种统一能源感知调控器，优化移动设备上LLM推理的能效，减少延迟并保持相同能耗。
...
Method: 通过测量现有LLM框架能效，分析调控器间交互问题，设计FUSE统一调控器。

Result: FUSE显著减少首令牌和每输出令牌延迟，同时保持相同能耗。
